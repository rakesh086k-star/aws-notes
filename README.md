...
| extend rgScope = tolower(id)
| project resourceGroup = name, subscriptionId, rgScope
| join kind=leftouter (
    resources
    | where type =~ "microsoft.authorization/locks"
    | extend lockScope = tolower(tostring(properties.scope))
...
) on $left.rgScope == $right.lockScope
